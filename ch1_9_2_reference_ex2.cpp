...
// PntAdder함수는 매개변수로 전달 받은 두 개 점의 x좌표끼리 덧셈, y좌표 끼리 덧셈 연산으로 새로운 Point를 초기화해서 반환
// 하는 함수이다.이 때 Point 구조체 변수는 new 연산자를 이용해야한다.
#include<iostream>
using namespace std;
...
//  PntAdder는 매개변수로 전달된 두 개의 Point 참조자의 x좌표를 더한 x 값과 
//  y좌표를 더한 y값을 갖는 Point 구조체를 새로 만들어 
//  새로 만든 참조자를 반환한다 
// 
...
Point& PntAdder(const Point& r1, const Point& r2) {
	Point* ptr = new Point;

	ptr->xpos = r1.xpos + r2.xpos;
	ptr->ypos = r1.ypos + r2.ypos;
	return *ptr;
}
